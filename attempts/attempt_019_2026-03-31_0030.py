# =============================================================================
# Attempt 019 — 2026-03-31 00:30
#
# CHANGE: Fix fast_mul — declare `a: QArray[QBit]` instead of `a: QNum`.
#
# BUG IN 018: `a[i]` raises "TypeError: 'QNum' object is not subscriptable".
#   QNum doesn't support bit-level indexing via [i]. Only QArray[QBit] does.
#   Fix: declare `a: QArray[QBit]`. QNum is compatible with QArray[QBit]
#   (proven by ec_scalar_mult_add which accepts QNum for k: QArray[QBit]).
#
# ALL OTHER LOGIC IDENTICAL TO ATTEMPT_018:
#   fast_mul uses explicit double-XOR Bennett trick:
#     allocate temp
#     for each bit i of a:
#       temp ^= subscript(tbl_i, b)                    # XOR-load
#       control(a[i], modular_add_inplace(p, temp, r)) # conditional add
#       temp ^= subscript(tbl_i, b)                    # XOR-unload
#     free(temp)
#
...
#
# PREVIOUS: attempt_018 (QNum not subscriptable), attempt_012 (105,554 CX)
# =============================================================================
...
def solve(num_bits: int) -> int:
    ...
    Like attempt_018 but fast_mul uses `a: QArray[QBit]` (not QNum) to allow
    bit-level indexing `a[i]` for control targets. Returns the recovered private key d.
    """
    assert num_bits in SUPPORTED_BITS, f"attempt_019 supports {SUPPORTED_BITS}, got {num_bits}"
    ...
    print(f"\n[attempt_019] {num_bits}-bit | p={p} | n={n} | p_bits={p_bits} | var_len={var_len}")
    print(f"  fast_mul: {p_bits} iters × (XOR-load, controlled add, XOR-unload)")
    ...
    # -----------------------------------------------------------------------
    # fast_mul: explicit double-XOR Bennett trick
    # -----------------------------------------------------------------------

    @qperm
    def fast_mul(a: QArray[QBit], b: Const[QNum], result: QNum) -> None:
        """
        result += a*b mod p  (result must start at 0).

        Schoolbook: a*b = sum_{i: a[i]=1} (2^i * b) mod p.
        Manual Bennett trick (no within_apply):
          allocate temp
          for each bit i:
            temp ^= subscript(tbl_i, b)                       # XOR-load
            control(a[i], modular_add_inplace(p, temp, result)) # conditional add
            temp ^= subscript(tbl_i, b)                       # XOR-unload (self-inverse)
          free(temp)

        Uses `a: QArray[QBit]` (not QNum) so a[i] indexing works.
        QNum is compatible with QArray[QBit] (same as ec_scalar_mult_add).
        """
        temp = QNum()
    ...
    stem = f"attempt_019_{num_bits}bit"
    ...
    import json
    meta = {
        "attempt": "attempt_019_2026-03-31_0030",
        "bits": num_bits,
        "qubits": n_qubits,
        "depth": depth,
        "cx": cx_count,
