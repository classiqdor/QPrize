# =============================================================================
# Attempt 021 — 2026-03-31 05:00
#
# CHANGE: Add optimization_parameter=DEPTH to Constraints.
#
# DISCOVERY: Classiq's Constraints default optimization_parameter is
#   NO_OPTIMIZATION. We've been using Constraints(max_width=500) which
#   leaves optimization_parameter=NO_OPTIMIZATION — the synthesizer was
#   never told to actively optimize depth/CX!
#
#   Setting optimization_parameter=OptimizationParameter.DEPTH tells the
#   synthesis engine to explicitly minimize circuit depth (strongly correlated
#   with CX count). This is distinct from optimization_level (0-3) which
#   controls the breadth of synthesis search.
#
# ALSO: transpilation_option=TranspilationOption.INTENSIVE (was AUTO_OPTIMIZE).
#   These are independent axes — optimization_level affects synthesis,
#   transpilation_option affects post-synthesis transpilation.
#
# EXPECTED: CX < 105,554 if depth optimization finds better gate schedules.
#   Synthesis time may increase due to depth-optimization search.
#
...
#
# PREVIOUS: attempt_012 (no optimization_parameter, 105,554 CX),
#           attempt_020 (opt_level=2, same CX — level has no effect)
# =============================================================================
...
from classiq.interface.generator.model.preferences.preferences import TranspilationOption
...
def solve(num_bits: int) -> int:
    ...
    Identical to attempt_012 except:
    - Constraints adds optimization_parameter=DEPTH (was NO_OPTIMIZATION)
    - transpilation_option=INTENSIVE (was AUTO_OPTIMIZE)

    Returns the recovered private key d.
    """
    assert num_bits in SUPPORTED_BITS, f"attempt_021 supports {SUPPORTED_BITS}, got {num_bits}"
    ...
    print(f"\n[attempt_021] {num_bits}-bit | p={p} | n={n} | p_bits={p_bits} | var_len={var_len}")
    print(f"  optimization_parameter=DEPTH + transpilation_option=INTENSIVE")
    ...
    # -----------------------------------------------------------------------
    # Synthesize (KEY CHANGE: optimization_parameter=DEPTH + INTENSIVE transpile)
    # -----------------------------------------------------------------------
    ...
    qmod = create_model(
        ...
        constraints=Constraints(
            max_width=500,
            optimization_parameter=OptimizationParameter.DEPTH,  # ← NEW: explicitly minimize depth
        ),
        preferences=Preferences(
            optimization_level=1,
            timeout_seconds=3600,
            transpilation_option=TranspilationOption.INTENSIVE,   # ← NEW: most aggressive transpilation
        ),
    )
    ...
    stem = f"attempt_021_{num_bits}bit"
    ...
    import json
    meta = {
        "attempt": "attempt_021_2026-03-31_0500",
        "bits": num_bits,
        "qubits": n_qubits,
        "depth": depth,
        "cx": cx_count,
