# =============================================================================
# Attempt 017 — 2026-03-31 00:10
#
# CHANGE: Fix attempt_016's runtime error in fast_mul.
#
# BUG IN 016: `inplace_xor(temp, subscript(...))` fails at runtime with
#   ClassiqInternalExpansionError: 'CParamScalar' object has no attribute
#   'get_handle_binding'. Root cause: subscript(tbl, b) returns a CParamScalar
#   (lookup result), not a quantum handle. `inplace_xor` only accepts quantum
#   handles. `result ^= subscript(tbl, b)` works because QNum.__ixor__ is
#   overloaded by Classiq to handle lookup results — it's NOT the same as
#   inplace_xor.
#
# FIX: Replace the lambda compute block with a `def` function per iteration.
#   `temp ^= subscript(...)` is a statement (not an expression), so it can
#   only appear in a function body — not inside a lambda or tuple. Defining
#   a named function per loop iteration allows the use of `^=`.
#
# ALL OTHER LOGIC IDENTICAL TO ATTEMPT_015/016:
#   fast_mul decomposes a*b mod p bit-by-bit:
#     result += sum_{i: a[i]=1} (2^i * b) mod p
#   Each bit i uses a 1D Const[QNum] lookup (no bind, like sq_lookup).
#
...
#
# PREVIOUS: attempt_016 (inplace_xor CParamScalar error),
#           attempt_012 (sq_lookup only, 105,554 CX)
# =============================================================================
...
def solve(num_bits: int) -> int:
    ...
    Like attempt_016 (fast_mul schoolbook) but with compute block as def (not
    lambda), allowing `temp ^= subscript(...)` which is a statement not an
    expression. Returns the recovered private key d.
    """
    assert num_bits in SUPPORTED_BITS, f"attempt_017 supports {SUPPORTED_BITS}, got {num_bits}"
    ...
    print(f"\n[attempt_017] {num_bits}-bit | p={p} | n={n} | p_bits={p_bits} | var_len={var_len}")
    print(f"  fast_mul: {p_bits} iters × (1D lookup + controlled modular_add_inplace)")
    ...
    def fast_mul(a: QNum, b: Const[QNum], result: QNum) -> None:
        ...
        FIX over attempt_016: uses a `def` function per iteration rather than
        a lambda, so `temp ^= subscript(...)` (a statement, not expression) is
        valid. QNum.__ixor__ handles the lookup result; inplace_xor does not.
        """
        temp = QNum()
    ...
    stem = f"attempt_017_{num_bits}bit"
    ...
    import json
    meta = {
        "attempt": "attempt_017_2026-03-31_0010",
        "bits": num_bits,
        "qubits": n_qubits,
        "depth": depth,
        "cx": cx_count,
