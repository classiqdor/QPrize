# =============================================================================
# Attempt 016 — 2026-03-31 00:00
#
# CHANGE: Fix attempt_015's syntax error in fast_mul.
#
# BUG IN 015: `temp ^= subscript(...)` is an augmented assignment statement
#   and cannot appear inside a tuple expression within a lambda. Python raises
#   SyntaxError at parse time. Fix: replace with `inplace_xor(temp, subscript(...))`
#   which is a function call (expression) and is valid inside a lambda tuple.
#
# ALL OTHER LOGIC IDENTICAL TO ATTEMPT_015:
#   fast_mul decomposes a*b mod p using bit-by-bit accumulation:
#     result += sum_{i: a[i]=1} (2^i * b) mod p
#   Each bit i:
#     1. within_apply compute: allocate temp, load (2^i * b) mod p via 1D Const lookup
#     2. within_apply body: control(a[i], modular_add_inplace(p, temp, result))
#
#   All 1D lookup tables use b as Const[QNum] — no bind needed.
#   Estimated ~1320 CX per fast_mul (4 iters × ~330 CX) vs modular_multiply 2527 CX.
#
...
#
# PREVIOUS: attempt_015 (syntax error), attempt_012 (sq_lookup only, 105,554 CX)
# =============================================================================
...
def solve(num_bits: int) -> int:
    ...
    Like attempt_015 (fast_mul schoolbook decomposition) but with the syntax
    error fixed: `temp ^= subscript(...)` replaced with
    `inplace_xor(temp, subscript(...))` inside the lambda tuple.
    Returns the recovered private key d.
    """
    assert num_bits in SUPPORTED_BITS, f"attempt_016 supports {SUPPORTED_BITS}, got {num_bits}"
    ...
    print(f"\n[attempt_016] {num_bits}-bit | p={p} | n={n} | p_bits={p_bits} | var_len={var_len}")
    ...
    def fast_mul(a: QNum, b: Const[QNum], result: QNum) -> None:
        ...
        FIX over attempt_015: uses inplace_xor(temp, subscript(...)) instead of
        `temp ^= subscript(...)` to avoid augmented-assignment statement in lambda.
        """
        temp = QNum()
    ...
    stem = f"attempt_016_{num_bits}bit"
    ...
    import json
    meta = {
        "attempt": "attempt_016_2026-03-31_0000",
        "bits": num_bits,
        "qubits": n_qubits,
        "depth": depth,
        "cx": cx_count,
