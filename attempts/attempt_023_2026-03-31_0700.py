# =============================================================================
# Attempt 023 — 2026-03-31 07:00
#
# CHANGE: optimization_parameter=TranspilerBasisGates.CX in Constraints.
#
# DISCOVERY: Constraints.optimization_parameter accepts TranspilerBasisGates.CX.
#   This tells the synthesis engine to directly minimize CX (CNOT) gate count —
#   the exact metric that matters for hardware fidelity.
#
#   Difference from attempt_021 (DEPTH optimization):
#   - DEPTH: minimize circuit depth (serial layers of gates)
#   - CX: minimize total 2-qubit (CNOT) gate count
#   These are different metrics! A wide parallel circuit has low depth but
#   many gates; a serial circuit has high depth but potentially fewer CX gates.
#
# EXPECTED: CX < 105,554 if the CX minimizer finds a better decomposition.
#   This is the most direct possible objective for our goal.
#
...
#
# PREVIOUS: attempt_012 (no opt_param, 105,554 CX),
#           attempt_021 (DEPTH opt_param, running)
# =============================================================================
...
def solve(num_bits: int) -> int:
    ...
    Identical to attempt_012 except optimization_parameter=TranspilerBasisGates.CX
    (directly minimize CX gate count — most relevant metric for hardware fidelity).
    Returns the recovered private key d.
    """
    assert num_bits in SUPPORTED_BITS, f"attempt_023 supports {SUPPORTED_BITS}, got {num_bits}"
    ...
    print(f"\n[attempt_023] {num_bits}-bit | p={p} | n={n} | p_bits={p_bits} | var_len={var_len}")
    print(f"  optimization_parameter=TranspilerBasisGates.CX (directly minimize CX)")
    ...
    # -----------------------------------------------------------------------
    # Synthesize (KEY CHANGE: optimization_parameter=CX — directly minimize CX gates)
    # -----------------------------------------------------------------------
    ...
    qmod = create_model(
        ...
        constraints=Constraints(
            max_width=500,
            optimization_parameter=TranspilerBasisGates.CX,  # ← Minimize CX count directly
        ),
    ...
    stem = f"attempt_023_{num_bits}bit"
    ...
    import json
    meta = {
        "attempt": "attempt_023_2026-03-31_0700",
        "bits": num_bits,
        "qubits": n_qubits,
        "depth": depth,
        "cx": cx_count,
