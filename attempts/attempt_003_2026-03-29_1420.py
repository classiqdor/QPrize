# =============================================================================
# Attempt 003 — 2026-03-29
#
# Goal: Let Classiq's synthesizer do more work — bump optimization_level from
#       0 to 1 and see how much CX reduction we get for free.
#
# Previous attempt (002B): optimization_level=0, which disables the synthesizer's
#   gate-level optimizer. The Cadmium source shows 5 adder implementations are
#   available (QFT, CDKM ripple-carry, CDKM shallow, + relative-phase variants).
#   At level 1, the synthesizer can pick the best adder and apply gate-level
#   reductions automatically.
#
# Change: optimization_level=0 → optimization_level=1
#
# Supported bit sizes: 4 (ripple-carry), 6 (QFT-space)
# Expected duration:   4-bit ~60s, 6-bit ~120s (synthesis takes longer)
# =============================================================================
...
def solve(num_bits: int) -> int:
    ...
    print(f"\n[attempt_003] {num_bits}-bit | n={generator_order} | d={known_d} | "
